...
class Board(object):
    ...
    def create(cls, n_players):
        """
        Create a Board and randomly allocate the territories.
    ...
    def hostile_neighbors(self, territory_id):
        ...
        return (t for t in self.data
                if (t.player_id != player_id and
                    t.territory_id in neighbor_ids))

    def friendly_neighbors(self, territory_id):
        """
        Create a generator of all territories
        neighboring a given territory, of which
        ...
        Returns:
            generator: Generator of tuples of the
            form (territory_id, player_id, armies).
        """
        player_id = self.owner(territory_id)
        neighbor_ids = risk.definitions.territory_neighbors[territory_id]
        return (t for t in self.data if (t.player_id
                == player_id and t.territory_id in neighbor_ids))
    ...
    def is_valid_path(self, path):
        ...
        1. For all territories V in the list (except the last one),
        the next territory W is in the neighbors if V.
        ...
        for i in range(len(path) - 1):
            ...
            neighbors = set(t.territory_id for t in
                            Board.neighbors(self, current_territory))
            if next_territory not in neighbors:
                return False
        return True
    ...
    def is_valid_attack_path(self, path):
        ...
        All valid attacks, therefore, will follow a path of
        starting on one player's territory and moving trough enemy
        territories.

        Formally, an attack path is a valid path
        satisfying the following two additional properties:
        1. An attack path must contain at least two territories;
        1. If the first territory is owned by player A,
        then no other territories in the path are also owned by A.
        ...
        first_territory = path[0]
        player_id = self.owner(first_territory)

        if player_id is not None:
            for territory in path[1:]:
                if self.owner(territory) == player_id:
                    return False
    ...
    def cost_of_attack_path(self, path):
        '''
        The cost of an attack path is the total number
        of enemy armies in the path.
        In other words, it is the total number of armies in the
        subpath starting at the second vertex.

        Args:
            path ([int]): a list of territory_ids which must be a
            valid attack path
    ...
    def shortest_path(self, source, target):
        ...
        This function does not take into account who owns
        the territories or how many armies are on the territories,
        and so a shortest path is simply a valid path with the
        smallest number of territories visited.
        ...
        Returns:
            [int]: a valid path between source and
            target that has minimum length;
            this path is guaranteed to exist
        '''
        dictionary = {}
        dictionary[source] = [source]
        q = deque([source])
        visited_territory = set([source])
        while q:
            current_territory = q.popleft()
            if source == target:
                return dictionary[source]
            for bor in risk.definitions.territory_neighbors[current_territory]:
                if bor not in visited_territory:
                    visited_territory.add(bor)
                    copydict = copy.deepcopy(dictionary)
                    copydict[current_territory].append(bor)
                    dictionary[bor] = copydict[current_territory]
                    if bor == target:
    ...
    def can_fortify(self, source, target):
        '''
        At the end of a turn, a player may choose to
        fortify a target territory by moving armies from a source territory.
        In order for this to be a valid move,
        there must be a valid path between the
        source and target territories that is
        owned entirely by the same player.
        ...
        Returns:
            bool: True if reinforcing the target
            from the source territory is a valid move
        '''
        dictionary = {}
        dictionary[source] = [source]
        q = deque([source])
        ...
        while q:
            ...
            if current_territory == target:
                return False
            for bor in risk.definitions.territory_neighbors[current_territory]:
                player_id = self.owner(bor)
                ownerid = self.owner(source)
                if player_id == ownerid and bor not in visited_territory:
                    visited_territory.add(bor)
                    copydict = copy.deepcopy(dictionary)
                    copydict[current_territory].append(bor)
                    dictionary[bor] = copydict[current_territory]
    ...
    def cheapest_attack_path(self, source, target):
        '''
        This function uses Dijkstra's algorithm to calculate a
        cheapest valid attack path between two territories if
        such a path exists.
        There may be multiple valid cheapest attack paths
        (in which case it doesn't matter which this function returns),
        ...
        Returns:
            [int]: a list of territory_ids representing the
            valid attack path;
        ...
        while q:
            current_territory, c_priority = q.popitem()
            if current_territory == target:
                return dictionary[current_territory]
            for bor in\
                    risk.definitions.territory_neighbors[current_territory]:
                if bor not in visited_territory and\
                        self.owner(bor) != self.owner(source):
                    copydict = copy.deepcopy(dictionary[current_territory])
                    copydict.append(bor)
                    dictionary[bor] = copydict
                    n_priority = c_priority + self.armies(bor)
                    if bor not in q:
                        dictionary[bor] = copydict
                        q[bor] = c_priority + self.armies(bor)
                    elif n_priority <= q[bor]:
                        dictionary[bor] = copydict
                        q[bor] = n_priority
                visited_territory.add(bor)
    ...
    def can_attack(self, source, target):
        ...
        Returns:
            bool: True if a valid attack path exists
            between source and target; else False
        '''
        dictionary = {}
        dictionary[source] = [source]
        q = deque([source])
    ...
    def n_continents(self, player_id):
        ...
        """
        return len([continent_id for continent_id in range(6)
                    if self.owns_continent(player_id, continent_id)])
    ...
    def num_foreign_continent_territories(self, continent_id, player_id):
        """
        Compute the number of territories owned
        by other players on a given continent.
        ...
        Returns:
            int: The number of territories on the
            continent owned by other players.
        """
        return sum(1 if t.player_id != player_id
                   else 0 for t in self.continent(continent_id))
    ...
    def possible_fortifications(self, player_id):
        ...
        return [Move(from_t.territory_id, from_t.armies,
                ...
                for from_t in self.mobile(player_id)
                for to_t in
                self.friendly_neighbors(from_t.territory_id)]
    ...
    def fortify(self, from_territory, to_territory, n_armies):
        ...
        Args:
            from_territory (int): Territory_id
            of the territory where armies leave.
            to_territory (int): Territory_id of
            the territory where armies arrive.
            n_armies (int): Number of armies to move.

        Raises:
            ValueError if the player moves too many or negative armies.
            ValueError if the territories do not
            share a border or are not owned by the same player.
        """
        if n_armies < 0 or self.armies(from_territory) <= n_armies:
            raise ValueError('Board: Cannot move {n} \
                             armies from territory {tid}.'
                             .format(n=n_armies, tid=from_territory))
        if to_territory not in [t.territory_id for t
                                in self.friendly_neighbors(from_territory)]:
            raise ValueError('Board: Cannot fortify,\
                             territories do not share owner and/or border.')
    ...
    def attack(self, from_territory, to_territory, attackers):
        ...
        Raises:
            ...
            ValueError if a player attacks himself
            or the territories do not share a border.

        Returns:
            bool: True if the defensive
            territory was conquered, False otherwise.
        """
        if attackers < 1 or self.armies(from_territory) <= attackers:
            raise ValueError('Board: Cannot attack with \
                             {n} armies from territory {tid}.'
                             .format(n=attackers, tid=from_territory))
        if to_territory not in [tid for (tid, _, _)
                                in self.hostile_neighbors(from_territory)]:
            raise ValueError('Board: Cannot attack, territories do not \
                             share border or are owned by the same player.')
    ...
    def plot_board(self, path=None, plot_graph=False, filename=None):
        ...
        Args:
            ...
            plot_graph (bool): if true, plots the graph
            structure overlayed on the board
            filename (str): if given, the plot will be
            saved to the given filename instead of displayed
        ...
        def plot_path(xs):
            ...
            for i, x in enumerate(xs[1:]):
                if (xs[i] == 19 and xs[i + 1] == 1)\
                        or (xs[i] == 1 and xs[i+1] == 19):
    ...
    def plot_single(territory_id, player_id, armies):
        ...
        plt.text(
            ...
            coor[1]*1.22 + 15,
            s=str(armies),
            color='black' if risk.definitions.player_colors[player_id] in
            ['yellow', 'pink'] else 'white',
            ha='center',
            size=15
            )
    ...
    def fight(cls, attackers, defenders):
        ...
        Returns:
            tuple (int, int): Number of lost attackers,
            number of lost defenders.
        """
        ...
        wins = [att_d > def_d for att_d,
                def_d in zip(attack_dices, defend_dices)]
    ...
    def mobile(self, player_id):
        """
        Create a generator of all territories
        of a player which can attack or move,
